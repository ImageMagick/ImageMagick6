...
WandExport DrawingWand *CloneDrawingWand(const DrawingWand *wand)
{
  ...
  (void) FormatLocaleString(clone_wand->name,MaxTextExtent,"DrawingWand-%.17g",
  ...
  if (base64 == (char *) NULL)
    {
      ...
      (void) FormatLocaleString(buffer,MaxTextExtent,"%.17g bytes",(double)
...
%                                                                             %
%                                                                             %
%   D r a w G e t E x c e p t i o n T y p e                                   %
%                                                                             %
%                                                                             %
%                                                                             %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
WandExport char *DrawGetVectorGraphics(DrawingWand *wand)
{
  ...
  if (child != (XMLTreeInfo *) NULL)
    {
      (void) FormatLocaleString(value,MaxTextExtent,"%.17g",(double)
  ...
  if (child != (XMLTreeInfo *) NULL)
    {
      (void) FormatLocaleString(value,MaxTextExtent,"%.17g",QuantumScale*
  ...
  const PaintMethod paint_method)
{
  ...
  if (wand->debug != MagickFalse)
    (void) LogMagickEvent(WandEvent,GetMagickModule(),"%s",wand->name);
  (void) MVGPrintf(wand,"matte %.17g %.17g '%s'\n",x,y,CommandOptionToMnemonic(
    MagickMethodOptions,(ssize_t) paint_method));
}

/*
...
WandExport MagickBooleanType DrawPopPattern(DrawingWand *wand)
{
  ...
  (void) FormatLocaleString(geometry,MaxTextExtent,"%.17gx%.17g%+.20g%+.20g",
  ...
  const double x_resolution,const double y_resolution)
{
  ...
  if (wand->debug != MagickFalse)
    (void) LogMagickEvent(WandEvent,GetMagickModule(),"%s",wand->name);
  (void) FormatLocaleString(density,MaxTextExtent,"%.17gx%.17g",x_resolution,
    y_resolution);
  (void) CloneString(&CurrentContext->density,density);
  return(MagickTrue);
}
...
WandExport DrawingWand *NewDrawingWand(void)
{
  ...
  (void) FormatLocaleString(wand->name,MaxTextExtent,"%s-%.17g",DrawingWandId,
    (double) wand->id);
  if (wand->debug != MagickFalse)
    (void) LogMagickEvent(WandEvent,GetMagickModule(),"%s",wand->name);
  wand->mvg=(char *) NULL;
