...
#define RenderPCLText  "  Rendering PCL...  "
  ...
  */
  ...
  (void) FormatLocaleString(geometry,MaxTextExtent,"%.17gx%.17g",(double)
  ...
  page.height=CastDoubleToUnsigned((double) page.height*image->y_resolution/
    delta.y+0.5);
  (void) FormatLocaleString(options,MaxTextExtent,"-g%.17gx%.17g ",(double)
     page.width,(double) page.height);
  image=DestroyImage(image);
  ...
  if (read_info->number_scenes != 0)
    {
      if (read_info->number_scenes != 1)
        (void) FormatLocaleString(options,MaxTextExtent,"-dLastPage=%.17g",
  ...
  */
  ...
  do
  {
    ...
    */
    ...
    (void) FormatLocaleString(buffer,MaxTextExtent,"\033*r%.17gs%.17gT",
      (double) image->columns,(double) image->rows);
    (void) WriteBlobString(image,buffer);
    (void) FormatLocaleString(buffer,MaxTextExtent,"\033*t%.17gR",(double)
      density);
    (void) WriteBlobString(image,buffer);
      ...
      if (image->storage_class == DirectClass)
      ...
      else
        {
          ...
          */
          ...
          {
            (void) FormatLocaleString(buffer,MaxTextExtent,"\033*v%.17gI",
              (double) i);
            (void) WriteBlobString(image,buffer);
          }
        }
    ...
    for (y=0; y < (ssize_t) image->rows; y++)
    {
      ...
      switch (compression)
      {
        case NoCompression:
        {
          (void) FormatLocaleString(buffer,MaxTextExtent,"\033*b%.17gW",
            (double) length);
          (void) WriteBlobString(image,buffer);
