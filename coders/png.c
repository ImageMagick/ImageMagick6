...
#define MNG_MAX_LOOP_COUNT 1000
#define MNG_MAX_LOOP_NESTING 256
#define MNG_MAX_LOOP_OPS 50000
  ...
  */
  if (object_id && !mng_info->frozen[object_id])
   ...
   /* Set more properties for identify to retrieve */
   {
     ...
     if (png_get_valid(ping,ping_info,PNG_INFO_oFFs))
       {
         (void) FormatLocaleString(msg,MaxTextExtent,"x_off=%.17g, y_off=%.17g",
...
#define QM Quantum
                ...
                if (logging != MagickFalse)
                  (void) LogMagickEvent(CoderEvent,GetMagickModule(),
                    "    Magnify the rows to %.17g",(double) large_image->rows);
                ...
                if (logging != MagickFalse)
                  (void) LogMagickEvent(CoderEvent,GetMagickModule(),
                    "    Magnify the columns to %.17g",(double) image->columns);
    ...
    if (image_count > 10*mng_info->image_found)
      {
        ...
        (void) ThrowMagickException(exception,GetMagickModule(),CoderError,
          "Linked list is corrupted, beginning of list not found","`%s'",
          image_info->filename);
  ...
  if (mng_info->ticks_per_second && mng_info->image_found > 1 &&
     ...
     (Image *) NULL)
    {
      ...
      (void) ThrowMagickException(exception,GetMagickModule(),CoderError,
        "image->next for first image is NULL but shouldn't be.","`%s'",
        image_info->filename);
    }
  ...
  if (mng_info->image_found == 0)
    {
      ...
      (void) ThrowMagickException(exception,GetMagickModule(),CoderError,
        "No visible images in file","`%s'",image_info->filename);
  ...
  if (logging != MagickFalse)
    {
      ...
      while (GetNextImageInList(image) != (Image *) NULL)
      {
        image=GetNextImageInList(image);
        (void) LogMagickEvent(CoderEvent,GetMagickModule(),
          "    scene %.17g delay=%.17g",(double) scene++,(double) image->delay);
      }
    }
...
ModuleExport size_t RegisterPNGImage(void)
{
  ...
  *version='\0';
  ...
  entry->magick_module=ConstantString("PNG");
   ...
   if (logging != MagickFalse)
     {
       ...
       (void) LogMagickEvent(CoderEvent,GetMagickModule(),
             "      image->matte=%.17g",(double) image->matte);
       (void) LogMagickEvent(CoderEvent,GetMagickModule(),
  ...
  if (logging != MagickFalse)
    {
     ...
     (void) LogMagickEvent(CoderEvent,GetMagickModule(),
        "    image_matte=%.17g",(double) image->matte);
     (void) LogMagickEvent(CoderEvent,GetMagickModule(),
        "    image->depth=%.17g",(double) image->depth);
     (void) LogMagickEvent(CoderEvent,GetMagickModule(),
   ...
   const ImageInfo *image_info,Image *image)
{
  ...
  if ((image_info->type != TrueColorMatteType) &&
    ...
    {
      ImageType type = IdentifyImageType(image,&image->exception);
