...
static MagickBooleanType ParseHvcCAtom(HEICItemProp *prop,
  ...
  do {
    ...
    if (img) {
      ...
      for (y = 0; y < 256; y++) {
        ...
        for (x = 0; x < 256; x++) {
          SetPixelGreen(q, ScaleCharToQuantum(*p1++));
          SetPixelBlue(q, ScaleCharToQuantum(*p2++));
          q++;
        }
      ...
      Image* resized_chroma = ResizeImage(chroma, 512, 512, TriangleFilter, 1.0, exception);
      ...
      for (y = 0; y < 512; y++) {
        ...
        for (x = 0; x < 512; x++) {
          SetPixelRed(q, ScaleCharToQuantum(*l));
      ...
      case ATOM('i', 'r', 'o', 't'):
        {
          ...
          SetImageProperty(image, "exif:Orientation", value);
        }
        break;
    }
  }
