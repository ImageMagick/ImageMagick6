...
static int hls_to_rgb(int hue, int lum, int sat)
{
    ...
    if (sat == 0) {
        ...
        B = (hue_to_rgb(Magic1, Magic2, (ssize_t) hue + (HLSMAX / 3)) * RGBMAX + (HLSMAX / 2)) / HLSMAX;
        R = (hue_to_rgb(Magic1, Magic2, hue) * RGBMAX + (ssize_t) (HLSMAX / 2)) / HLSMAX;
        G = (hue_to_rgb(Magic1, Magic2, (ssize_t) hue - (HLSMAX / 3)) * RGBMAX + (HLSMAX/2)) / HLSMAX;
    }
                               ...
                               size_t                     /* out */ *ncolors    /* palette size (<= 256) */)
{
    ...
    imsx = 2048;
    imsy = 2048;
    if (SetImageExtent(image,imsx,imsy) == MagickFalse)
      return(MagickFalse);
        ...
        if ((p[0] == '\033' && p[1] == 'P') || *p == 0x90) {
            ...
            if (*p == 'q') {
                ...
                if (n > 2) {        /* Pn3 */
                    ...
                    attributed_pan = (int) (((size_t) attributed_pan * param[2]) / 10);
                    attributed_pad = (int) (((size_t) attributed_pad * param[2]) / 10);
