...
static inline MagickBooleanType AcquireKernelValues(KernelInfo *kernel)
{
  ...
  return(kernel->values == (double *) NULL ? MagickFalse : MagickTrue);
}
  ...
  while ( method_loop < method_limit && method_changed > 0 ) {
    ...
    while ( norm_kernel != NULL ) {
      ...
      while ( stage_loop < stage_limit ) {
        ...
        if ( verbose != MagickFalse ) {
          if ( stage_limit > 1 )
            (void) FormatLocaleString(v_info,MaxTextExtent,"%s:%.17g.%.17g -> ",
             CommandOptionToMnemonic(MagickMorphologyOptions,method),(double)
             method_loop,(double) stage_loop);
          else if ( primitive != method )
            (void) FormatLocaleString(v_info, MaxTextExtent, "%s:%.17g -> ",
              CommandOptionToMnemonic(MagickMorphologyOptions, method),(double)
              method_loop);
          else
            v_info[0] = '\0';
