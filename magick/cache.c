  ...
  ExceptionInfo *exception)
{
  ...
  source_info.file=(-1);
  (void) FormatLocaleString(cache_info->filename,MaxTextExtent,"%s[%.17g]",
              ...
              if (cache_info->debug != MagickFalse)
                {
                  ...
                  type=CommandOptionToMnemonic(MagickCacheOptions,(ssize_t)
                    cache_info->type);
                  (void) FormatLocaleString(message,MaxTextExtent,
                    "open %s (%s %s, %.17gx%.17g %s)",cache_info->filename,
              ...
              if (cache_info->debug != MagickFalse)
                {
                  ...
                  type=CommandOptionToMnemonic(MagickCacheOptions,(ssize_t)
                    cache_info->type);
                  (void) FormatLocaleString(message,MaxTextExtent,
                    "open %s (%s[%d], %s, %.17gx%.17g %s)",cache_info->filename,
  ...
  */
  number_pixels=(MagickSizeType) width*height;
  {
    ...
    packets=MagickMax(number_pixels,MagickMax(cache_info->columns,
      cache_info->rows));
    if ((CacheOverflowSanityCheckGetSize(packets,1,&length) != MagickFalse) ||
    ...
    if (cache_info->active_index_channel != MagickFalse)
      {
        if ((CacheOverflowSanityCheckGetSize(number_pixels,sizeof(IndexPacket),&extent) != MagickFalse) ||
